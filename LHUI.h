...
namespace LHUI 
{
	...
	double CreateButton(const char* buttonText, double x, double y, double depth=-200.0)
	{
	...
	double CreateSlider(double x, double y, double width, double depth = -200.0, int steps = -1)
	{
		...
		Binds::CallBuiltinA("variable_instance_set", { sliderRef, "text", (double)steps });
		return sliderRef;
	}
}